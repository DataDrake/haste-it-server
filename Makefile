...
build: setup
    @$(call stage,BUILD)
    @$(GOCC) install -v github.com/DataDrake/haste-it-server
    @$(call pass,BUILD)
...
install:
    @$(call stage,INSTALL)
    install -D -m 00755 $(GOBIN)/haste-it-server $(DESTDIR)$(BINDIR)/haste-it-server
    @$(call pass,INSTALL)

uninstall:
    @$(call stage,UNINSTALL)
    rm -f $(DESTDIR)$(BINDIR)/haste-it-server
    @$(call pass,UNINSTALL)

clean:
    @$(call stage,CLEAN)
    @$(call task,Removing symlinks...)
    @unlink $(PROJROOT)/haste-it-server
